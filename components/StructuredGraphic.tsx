...
export function StructuredGraphic() {
  return (
    <div className="relative mx-auto w-full max-w-[620px] rounded-[2rem] border border-white/10 bg-white/[0.055] p-4 shadow-[0_35px_100px_rgba(0,0,0,0.35)] backdrop-blur sm:p-6">
      <div className="flex items-center justify-between border-b border-white/10 pb-4">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/35">AI operations architecture</p>
      </div>

      <div className="relative grid gap-4 py-6 sm:grid-cols-[0.8fr_1.15fr_0.9fr] sm:items-center sm:gap-3">
        <div className="relative z-10 space-y-3">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.16em] text-white/35">Business systems</p>
          {systems.map((system) => (
        ...
        <div className="relative z-10 rounded-2xl border border-blue-300/25 bg-gradient-to-br from-brand/80 to-blue-900/80 p-5 shadow-[0_0_60px_rgba(37,99,235,0.25)]">
          <div className="flex items-center justify-between">
            ...
            <span className="flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-wider text-green-200">
        ...
        <div className="relative z-10 space-y-3">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.16em] text-white/35">Outcomes</p>
      ...
      <div className="grid grid-cols-3 gap-2 border-t border-white/10 pt-4 text-center">
        {["Connected", "Controlled", "Measurable"].map((item) => (
          <p key={item} className="text-[9px] font-bold uppercase tracking-[0.12em] text-white/30">{item}</p>
        ))}
      </div>
    </div>
  );
